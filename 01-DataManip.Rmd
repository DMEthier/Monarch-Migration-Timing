...
#open<-open %>% filter(SiteCode=="LPBO1")
open<-format_dates(open) #creates date and doy variable
open<-open %>% filter(doy>200) #remove spring counts 
...
LPBO1 <- data %>% 
    ...
    dplyr::summarize (mean.date = weighted.mean(doy, ObservationCount), weighted.var=wtd.var(doy, ObservationCount), quant95 = quantile(doy, probs = 0.95)) %>%     mutate(weighted.sd=sqrt(weighted.var), sd.low = mean.date-weighted.sd, sd.high = mean.date+weighted.sd) 
...
ggplot(data1, aes(x=doy, y=Obs1)) +  
  ...
  geom_vline(data = LPBO1, aes(xintercept = quant95), color="blue") +
